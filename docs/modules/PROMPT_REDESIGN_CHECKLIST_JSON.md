1. Возьми @/docs/modules/01-physical-portrait/ACTOR_PHYSICAL_CHECKLIST.md  файл и напиши для такого файла js-конвертер md to JSON. 
2. Сопоставь данные в md с примером JSON ниже — я сделал точное сопоставление данных.
3. JSON должен иметь жесткую иерархию: n секций -> m подсекций -> x групп -> y вопросов -> z ответов
4. Каждая сущность имеет свой жестко заданный тип — смотри в описание типов ниже
5. Каждая сущность должна иметь человеко-читаемый id — смотри в пример кода ниже
6. Если тебе непонятно, как некие данные из md должны попасть в JSON, спроси меня, уточни
