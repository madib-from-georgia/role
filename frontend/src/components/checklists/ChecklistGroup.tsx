...
export const ChecklistGroup: React.FC<ChecklistGroupProps> = ({
  ...
  const getGroupProgress = () => {
    ...
    const groupProgress = checklists.map(checklist => {
      const checklistProgress = progress?.find(p => p.checklist_id === checklist.id);
      return checklistProgress?.completion_percentage || 0;
    });
    
    return Math.round(groupProgress.reduce((sum, p) => sum + p, 0) / groupProgress.length);
  };
  ...
  return (
    <div className={`checklist-group ${!unlocked ? 'checklist-group--locked' : ''}`}>
      <div className="checklist-group__header">
        <div className="checklist-group__title-row">
          ...
          <div className="checklist-group__progress-badge">
            {groupProgress}%
          </div>
        </div>
              ...
              >
                ...
                <div className="checklist-item__content">
                  ...
                  {checklist.description && (
                    <p className="checklist-item__description">{checklist.description}</p>
                  )}
