...
def reset_database():
    ...
    try:
        ...
        # 4. Создаем mock пользователя
        print("\n👤 Создаю mock пользователя...")
        result = subprocess.run(
        ...
        if result.returncode == 0:
            print("✅ Mock пользователь создан успешно")
        else:
            print(f"❌ Ошибка при создании mock пользователя: {result.stderr}")
            return False
        
        # 5. Проверяем, что база данных создана
        if db_file.exists():
        ...
        print("  - Создан mock пользователь для разработки")
